...
class Payment_model extends CI_Model {
	public function get_payments(array $payment_params = NULL){
		...
		if(!empty($payment_params['user_id'])){
			$where_condition .= "AND A.user_id = " . $payment_params['user_id'] . "";
		}
		...
		$query = "
			SELECT 
				...
				A.event_id, C.event_name, C.event_details,
				IF( ISNULL(D.event_img), '', CONCAT('" . base_url() . "uploads/events/', D.event_img) ) as event_image,
				C.event_venue, C.event_fee, 
			...
			LEFT JOIN 
				event_images D ON A.event_id = D.event_id
			WHERE 
